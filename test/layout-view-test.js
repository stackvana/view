...
test("should be able to perform deeply nested layout", function (t) {
  view.create( { path: __dirname + "/nested-layouts" } , function(err, _view) {
    t.error(err, 'no error');
    t.ok(_view, 'view is returned');
    _view.subview.deepview.index.present({}, function (err, result) {
      t.error(err, 'no error');
      t.ok(result, 'present returns result');
      console.log(result.red)
      t.equal(result,
        '<div class="outer"><div class="yield"><div class="inner"><div class="yield"><div class="deepest"><div class="yield"><h2>Deep level</h2></div></div></div></div></div></div>');
      t.end();
    });
  });
});
...
test("deeply nested layouts should use all layout presenters", function (t) {
  view.create( { path: __dirname + "/nested-layouts-presenters" } , function(err, _view) {
    t.error(err, 'no error');
    t.ok(_view, 'view is returned');
    _view.subview.deepview.index.present({}, function (err, result) {
      ...
      t.ok(result, 'present returns result');
      t.equal(result,
        '<div class="outer"><span class="foo">bar</span><div class="yield"><div class="inner"><span class="hello">there</span><div class="yield"><h2>Deep level</h2></div></div></div></div>');
      t.end();
    });
  });
});
