...
//
// Creates static representation of view for a ./public folder
//
View.prototype.generateStatic = require('./generateStatic');
...
View.prototype.present = function (options, callback) {
  ...
  if (self.name !== "layout") {
    ...
    if (useLayout && !overrideLayout) {
      var _before= findBefore(self);

      if (_before && self.name !== '_before' && self.name !== '_after') {
        _before.present(options, function(err, processed){
          layoutAndPresent()
        })
      } else {
        layoutAndPresent();
      ...
      function layoutAndPresent() {
        layout.call(self, self, options, function (err, result) {
          ...
          return _presenter.call(self, options, function (err, result){
            ...
            var _after = findAfter(self);
            if (_after && self.name !== '_before' && self.name !== '_after') {
              _after.present({ view: self, html: result }, function(err, processed){
    ...
    // console.log('about to call', self.name, self.template)
    return _presenter.call(self, options, function(err, result) {
      if (saveToCache) {
        self.lastCache = new Date().getTime();
        self._cache = result;
...
function findAfter (view) {
  ...
  if (typeof view._after !== "undefined" && typeof view._after.presenter === "function") {
    return view._after;
  }
  ...
  if (typeof view.parent._after !== "undefined") {
    return view.parent._after;
  }

  // there is a parent, but it doesn't contain a valid layout.
  // try finding a new layout up one level of the View tree
  return findAfter(view.parent);

};

function findBefore (view) {
  ...
  // if the current level of the view contains a layout, return that
  if (typeof view._before !== "undefined" && typeof view._before.presenter === "function") {
    return view._before;
  }
  ...
  // and there is a parent view, and it has a layout...
  // return that parent layout
  if (typeof view.parent._before !== "undefined") {
    return view.parent._before;
  }

  // there is a parent, but it doesn't contain a valid layout.
  // try finding a new layout up one level of the View tree
  return findBefore(view.parent);
...
View.prototype.replaceRelativeLinks = function replaceRelativeLinks (root, key, html) {
  // console.log('replaceRelativeLinks', root, key)
  key = key.replace('/index', '/');
  var $ = query(html);
  $('a').each(function() {
    var href = $(this).attr('href');
    if (href.substr(0,2) === "./") {
        href = root + key + href.substr(2, href.length - 1);
    }
    $(this).attr('href', href);
  });
  return $.html();
...
View.prototype.absLinks = function absLinks (opts) {
  var $ = this.$;
  var key = this.key.replace('/index', '/');
  $.html(replaceRelativeLinks(root, key, $.html()))
  return $;
}
